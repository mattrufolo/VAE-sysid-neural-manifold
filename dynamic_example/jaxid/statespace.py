...
class StateUpdateMLP(nn.Module):
    ...
    mlp_scale: float = 1e-2
    resnet_scale: float = 0.0
    ...
    def setup(self):
        ...
        self.net = MLP(self.features, last_layer_kwargs=self.last_layer_kwargs) 
        #last_layer_kwargs={"kernel_init": kernel_init, "bias_init": bias_init})

    def __call__(self, x, u):
        new_state = self.mlp_scale * self.net(jnp.r_[x, u])
        if self.resnet_scale > 0:
            new_state = new_state + self.resnet_scale*x
        #new_state = self.scale * self.net(jnp.r_[x, u]) + x
        #new_state = self.net(jnp.r_[x, u])
        return new_state  
